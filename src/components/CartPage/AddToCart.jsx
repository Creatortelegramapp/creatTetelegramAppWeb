...
export default function AddToCartButton({ productId, onAddToCart, initialIsAdded }) {
    const [isAdded, setIsAdded] = useState(initialIsAdded);

    useEffect(() => {
        const storedCart = JSON.parse(localStorage.getItem("cart")) || [];
        setIsAdded(storedCart.includes(productId));
    }, [productId]);

    const handleAddToCart = (e) => {
        e.preventDefault();
        const storedCart = JSON.parse(localStorage.getItem("cart")) || [];
        const updatedCart = storedCart.includes(productId) ? storedCart : [...storedCart, productId];
